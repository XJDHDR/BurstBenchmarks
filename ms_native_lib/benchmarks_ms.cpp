...
BOOL APIENTRY benchmarks_ms( HMODULE hModule,
...
extern "C" EXPORT uint32_t benchmark_fibonacci(uint32_t number) {
	if (number <= 1)
		return 1;
...
extern "C" EXPORT float benchmark_mandelbrot(uint32_t width, uint32_t height, uint32_t iterations) {
	float data = 0.0f;
...
extern "C" EXPORT double benchmark_nbody(uint32_t advancements) {
	NBody sun[5] = { 0 };
	NBody* end = sun + 4;
...
extern "C" EXPORT uint32_t benchmark_sieve_of_eratosthenes(uint32_t iterations) {
	const int size = 1024;
...
extern "C" EXPORT float benchmark_pixar_raytracer(uint32_t width, uint32_t height, uint32_t samples) {
	marsagliaZ = 666;
	marsagliaW = 999;
...
extern "C" EXPORT float benchmark_fireflies_flocking(uint32_t boids, uint32_t lifetime) {
	parkMiller = 666;
	maxSpeed = 1.0f;
	maxForce = 0.03f;
...
extern "C" EXPORT float benchmark_polynomials(uint32_t iterations) {
	const float x = 0.2f;
...
extern "C" EXPORT float benchmark_particle_kinematics(uint32_t quantity, uint32_t iterations) {
...
extern "C" EXPORT int benchmark_arcfour(uint32_t iterations) {
...
extern "C" EXPORT uint64_t benchmark_seahash(uint32_t iterations) {
...
extern "C" EXPORT int benchmark_radix(uint32_t iterations) {
	classicRandom = 7525;
